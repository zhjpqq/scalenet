...
class RockBlockU(RockBlock):
    # 第1/2/3分支分别独立处理图像
    def __init__(self, depth=16, branch=3, expand=(0, 0), dataset='cfar'):
        ...
        if dataset == 'cifar':
            ...
            if branch >= 2:
                self.branch2 = nn.Conv2d(3, depth + expand[0], kernel_size=3, stride=2, padding=1, bias=False)
            if branch >= 3:
                self.branch3 = nn.Sequential(
                    nn.Conv2d(3, depth + expand[0] + expand[1], kernel_size=3, stride=2, padding=1, bias=False),
            ...
            if branch >= 2:
                self.branch2 = nn.Sequential(
                    nn.Conv2d(3, depth + expand[0], kernel_size=7, stride=2, padding=3, bias=False),
                    nn.BatchNorm2d(depth + expand[0]),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(kernel_size=4, stride=4, padding=0),
                )
            if branch >= 3:
                self.branch3 = nn.Sequential(
                    nn.Conv2d(3, depth + expand[0] + expand[1], kernel_size=7, stride=2, padding=3, bias=False),
                    nn.BatchNorm2d(depth + expand[0] + expand[1]),
                    nn.ReLU(inplace=True),    # ??
                    nn.MaxPool2d(kernel_size=4, stride=4, padding=0),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
                )
        ...
        if dataset == 'cifar':
            ...
            if branch >= 2:
                self.branch2 = nn.Sequential(
                    nn.BatchNorm2d(depth),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(depth, depth + expand[0], kernel_size=2, stride=2, padding=0, bias=False)
                )
            if branch >= 3:
                self.branch3 = nn.Sequential(
                    nn.BatchNorm2d(depth),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(depth, depth + expand[0] + expand[1], kernel_size=2, stride=2, padding=0, bias=False),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
                )
        elif dataset == 'imagenet':
            self.branch1 = nn.Sequential(
...
class RockBlockR(RockBlock):
    ...
    def __init__(self, depth=16, branch=3, expand=(0, 0), dataset='cfar'):
        ...
        if dataset == 'cifar':
            self.branch1 = nn.Conv2d(3, depth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 2:
                self.branch2 = nn.Conv2d(3, depth + expand[0], kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 3:
                self.branch3 = nn.Conv2d(3, depth + expand[0] + expand[1], kernel_size=3, stride=1, padding=1, bias=False)
        elif dataset == 'imagenet':
            self.branch1 = nn.Sequential(
                nn.Conv2d(3, depth, kernel_size=7, stride=2, padding=3, bias=False),
                nn.BatchNorm2d(depth),
            ...
            if branch >= 2:
                self.branch2 = nn.Sequential(
                    nn.Conv2d(3, depth + expand[0], kernel_size=5, stride=2, padding=2, bias=False),
                    nn.BatchNorm2d(depth + expand[0]),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0),
                )
...
class RockBlockX(nn.Module):
    ...
    """
    def __init__(self, outdepth=16, branch=4, dataset='cfar'):
        super(RockBlockX, self).__init__()
        self.outdepth = outdepth
        self.branch = branch
        if dataset == 'cifar':
            self.branch1 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 2:
...
class RockBlockQ(nn.Module):
    ...
    def __init__(self, outdepth=16, branch=4, dataset='cfar'):
        ...
        if dataset == 'cifar':
            self.branch1 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 2:
                self.branch2 = nn.Conv2d(3, outdepth, kernel_size=3, stride=2, padding=1, bias=False)
            if branch >= 3:
                self.branch3 = self.branch3 = nn.Sequential(
                    nn.Conv2d(3, outdepth, kernel_size=3, stride=2, padding=1, bias=False),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
            if branch >= 4:
            ...
            if branch >= 4:
                self.branch4 = nn.Sequential(
                    nn.Conv2d(3, outdepth, kernel_size=7, stride=4, padding=3, bias=False),
                    nn.BatchNorm2d(outdepth),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
...
class RockBlockO(nn.Module):
    ...
    def __init__(self, outdepth=16, branch=4, dataset='cfar'):
        ...
        if dataset == 'cifar':
            self.branch1 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 2:
                self.branch2 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 3:
                self.branch3 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            if branch >= 4:
                self.branch4 = nn.Conv2d(3, outdepth, kernel_size=3, stride=1, padding=1, bias=False)
            ...
            if branch >= 2:
                self.branch2 = nn.Sequential(
                    nn.Conv2d(3, outdepth, kernel_size=5, stride=2, padding=2, bias=False),
                    nn.BatchNorm2d(outdepth),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
            if branch >= 3:
...
############################################################
"""  RockBlock with 3 branchses  for  WaveResNet 
     Not Support BottleNeck， Nown deprecated !!!!
"""
############################################################
