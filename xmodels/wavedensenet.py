...
"""
  WaveDenseNet => RockBlock + DoubleCouple + SingleCouple + Summary(+merge +split) + BottleNeck(暂无)
"""
...
class DoubleCouple(nn.Module):
    ...
    def __init__(self, indepth, growth, active='relu', first=False, after=True, down=False, trans='A', reduce=0.5,
        ...
        self.deconv3 = nn.ConvTranspose2d(indepth + growth, growth, 4, stride=2, padding=1,
                                          output_padding=0, bias=False, dilation=1)
        self.bn4 = nn.BatchNorm2d(indepth + growth)
        self.deconv4 = nn.ConvTranspose2d(indepth + growth, growth, 4, stride=2, padding=1,
                                          output_padding=0, bias=False, dilation=1)
...
class SingleCouple(nn.Module):
    ...
    def __init__(self, indepth, growth, active='relu', first=False, after=True, down=False, trans='A',
        ...
        self.conv1 = nn.Conv2d(indepth, first_outdepth, 3, stride=2, padding=1, bias=False, dilation=1)
        self.bn2 = nn.BatchNorm2d(indepth + growth)
        self.deconv2 = nn.ConvTranspose2d(indepth + growth, growth, 4, stride=2, padding=1,
                                          output_padding=0, bias=False, dilation=1)
...
class WaveDenseNet(nn.Module):
    ...
    def __init__(self, stages=4, rock='O', branch=4, indepth=16, growth=12, multiway=4,
        ...
        for i in range(stages):
            dense_layer = self._make_dense_layer(self._couple[blocks[i]], layers[i], indepth, growth,
                                                 classify[i], self.after[i], trans[i], reduction[i],
                                                 last_branch, last_down, last_expand, i)
            setattr(self, 'dense%s' % (i+1), dense_layer)
    ...
    def _make_dense_layer(self, block, nums, indepth, growth, cfy, after, trans, reduce,
                          last_branch, last_down, last_expand, stage):
        layers = []
        for i in range(nums - 1):
            if self.multiway == 3: first = True                             # 固定所有block内第1个conv1层的outdepth
            elif self.multiway == 4: first = bool(stage == 0 and i == 0)    # 固定第1个block内的第1个conv1层的outdepth
