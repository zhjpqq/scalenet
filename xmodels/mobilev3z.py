...
"""
https://github.com/kuan-wang/pytorch-mobilenet-v3.git
"""
...
def MobileV3Z(mode='large'):
    return MobileNetV3(mode=mode)
...
if __name__ == '__main__':
    ...
    model = mobilenetv3(pretrained=False, mode='large')
    model = mobilenetv3(pretrained=False, mode='small')

    # ckpt = ['mobilev3_z_small-.pth', 'mobilev3_z_large-.pth'][1]
    # state_dict = torch.load('/data/zhangjp/PreTrainedModels/%s' % ckpt)
    # model.load_state_dict(state_dict)

    print('\n', model, '\n')
    # utils.tensorboard_add_model(model, x)
    xtils.calculate_params_scale(model, format='million')
    # utils.calculate_FLOPs_scale(model, use_gpu=False, input_size=224, multiply_adds=True)
