...
if __name__ == '__main__':
    ...
    model = [MobileV3D('large'), MobileV3D('small')][1]
    print('\n', model, '\n')

    ckpt = ['mobilev3_d_large.pth.tar', 'mobilev3_d_small.pth.tar'][1]
    ckpt = os.path.join('/data/zhangjp/PreTrainedModels', ckpt)
    model = xtils.load_ckpt_weights(model, ckptf=ckpt, device='cpu', mgpus_to_sxpu='m2s')
